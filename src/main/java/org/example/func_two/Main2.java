...
//        int level =  item.getLevel() == null ? 1 : item.getLevel() + 1;
...
//                    OtherInfo2 lastOne = findOne.get(findOne.size() - 1);
//                    result.add(lastOne);
                }
                otherInfo2s.addAll(otherInfo2sup.isEmpty() ? otherInfo2slow : otherInfo2sup);
//                for (int i = 1; i < otherInfo2s.size(); i++) {
//                    OtherInfo2 levelItem = otherInfo2s.get(i);
//                    levelItem.setLevel(level + 1);
//                }

//                if (otherInfo2s.isEmpty()) {
//                    // 证明无法再有子集，那么就是当前能够借贷相抵的数据
//                    result.add(otherInfo2);
//                } else {
