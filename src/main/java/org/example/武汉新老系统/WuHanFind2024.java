...
@Service
public class WuHanFind2024 {
    @Resource
    private JdbcTemplate jdbcTemplate;
    @Resource
    private WuHanStep6 wuHanStep6Test;
    @Resource
    private WuHanFindUtil wuHanFindUtil;
    @Resource
    private WuHanStep5 wuHanStep5;
    public List<OracleData> find( String selectCompanyName){
        Map<String, List<NewBalanceExcelResult>> listMap = new HashMap<>();
        EasyExcel.read("src/main/java/org/example/excel/zhong_nan/merge/wuhan/"+ selectCompanyName +"最终组合结果-2023-余额表.xlsx", NewBalanceExcelResult.class, new PageReadListener<NewBalanceExcelResult>(dataList -> {
...
//            );
        ...
        companyList.add("江苏中南物业服务有限公司潜江分公司");
        companyList.add("江苏中南物业服务有限公司武汉分公司");
            for (String newCompanyName : companyList) {
                ...
                if (newCompanyName.equals("江苏中南物业服务有限公司张家港分公司")){
                    List<OracleData> step5Result = wuHanStep5.step5Test(newCompanyName)
                            .stream()
                            ...
                            .filter(item -> wuHanFindUtil.isBackProject(item.getActualProject()))
                            .collect(Collectors.toList());
                    List<Step6OldDetailExcel> list = new ArrayList<>();
                    EasyExcel.read("src/main/java/org/example/excel/zhong_nan/wuhan/file/2024发生额.xlsx",
                            Step6OldDetailExcel.class,
                            new PageReadListener<Step6OldDetailExcel>(list::addAll)
                    );
                    WuHanStep6.Step6TestResult step6TestResult = wuHanStep6Test.step6Test(newCompanyName, step5Result,list);
                    // 旧系统处理后数据
                    ...
                    for (Step6OldDetailExcel data : oldDataList) {
                        OracleData oracleData = wuHanFindUtil.coverStep6OldDetailExcelToOracleData(data,"24年1-9月旧系统序时账");
...
//                    List<OracleData> newDataList = jdbcTemplate.query(findSql, new BeanPropertyRowMapper<>(OracleData.class));
                    List<OracleData> newDataList = new ArrayList<>();
                    EasyExcel.read("src/main/java/org/example/excel/zhong_nan/merge/wuhan/"+newCompanyName+".xlsx", OracleData.class, new PageReadListener<OracleData>(dataList -> {
                        for (OracleData demoData : dataList) {
                            newDataList.add(demoData);
                        }
                    })).sheet().doRead();
...
//        }
        ...
        EasyExcel.write( "src/main/java/org/example/excel/zhong_nan/merge/wuhan/"+ selectCompanyName +"-最终组合结果-2024-余额表.xlsx", NewBalanceExcelResult.class).sheet("余额表").doWrite(finalExcel);
        return xsList;
    }
