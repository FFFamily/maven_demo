    ...
    public List<OracleData> find(List<Step6OldDetailExcel> list, String newCompanyName){
        ...
        List<OracleData> step5Result = wuHanStep5.step5Test(newCompanyName)
                ...
                for (Step6OldDetailExcel data : oldDataList) {
                    OracleData oracleData = wuHanFindUtil.coverStep6OldDetailExcelToOracleData(data,"23年7-12月旧系统序时账");
                ...
                File file = new File("src/main/java/org/example/excel/zhong_nan/merge/wuhan/company_2023_6_12/"  + newCompanyName + "-2023-1-6-组合序时账.xlsx");
                if (file.exists()){
                    EasyExcel.read(file,
                            OracleData.class,
                ...
                List<OracleData> list2 = map2.getOrDefault(newCompanyName, new ArrayList<>());
...
//        }
        ...
        EasyExcel.write( "src/main/java/org/example/excel/zhong_nan/merge/wuhan/"+ newCompanyName +"最终组合结果-2023-余额表.xlsx", NewBalanceExcelResult.class).sheet("余额表").doWrite(finalExcel);
        return xsList;
    }
    ...
    public Map<String, List<NewBalanceExcelResult>>  initBalance(String str){
        ...
        EasyExcel.read("src/main/java/org/example/excel/zhong_nan/merge/wuhan/"+str+"-最终组合结果-2022-余额表.xlsx", NewBalanceExcelResult.class, new PageReadListener<NewBalanceExcelResult>(dataList -> {
