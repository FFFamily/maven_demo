...
public class Step5 {
    ...
    public List<OracleData> step5Test(String newCompanyName){
        String findPiSQL = "SELECT  * FROM ZDPROD_EXPDP_20241120 z WHERE z.\"公司段描述\" = '"+newCompanyName+"' AND z.\"期间\" >= '2023-07' AND z.\"期间\" <= '2023-12' ";
        List<OracleData> sqlList = jdbcTemplate.query(findPiSQL, new BeanPropertyRowMapper<>(OracleData.class))
                .stream()
                .peek(this::data)
                .collect(Collectors.toList());
        Map<String, List<OracleData>> map = sqlList
                .stream()
                .collect(Collectors.groupingBy(OracleData::get批名));
        // 拿到所有的行说明
        for (String pi : map.keySet()) {
            System.out.println(pi);
            List<OracleData> mapList = map.get(pi).stream().filter(item -> {
                String form = (String) item.get科目段描述();
